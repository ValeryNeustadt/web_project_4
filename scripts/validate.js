...
const resetValidity = (props) => {
  const { formSelector, inputSelector, errorClass, inputErrorClass } = props;
  const forms = Array.from(document.querySelectorAll(formSelector));
  forms.forEach((form) => {
    const inputs = form.querySelectorAll(inputSelector);
    inputs.forEach((inputItem) => {
      const error = document.querySelector(
        `${inputSelector}-${inputItem.name}-error`
      );
...
const toggleButtonState = (props) => {
  const { inactiveButtonClass, buttonElement, inputs } = props;
